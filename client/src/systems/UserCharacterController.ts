...
const USER_CHARACTER_SPEED_PX_PER_SEC = 300;
const POSITION_SYNC_INTERVAL_MS = 200;

export type UserCharacterMode = "avatar" | "god";

export interface UserCharacterData {
  id: string;
  name: string;
  mode: UserCharacterMode;
  location: string;
...
export class UserCharacterController {
  ...
  mode: UserCharacterMode = "avatar";
  private isMoving = false;
  private targetPosition: { x: number; y: number } | null = null;
  private clickNavigationGeneration = 0;
  ...
  /** WebSocket 连接时收到的用户角色数据 */
  private playerData: UserCharacterData | null = null;
  private pendingFocusPulse = false;
  private spriteGeneration = 0;
  private handleSetUserCharacterMode = (mode: UserCharacterMode) => {
  ...
  private handlePointerDown = (
    ...
    if (
      ...
      || worldPoint.x < 0
      || worldPoint.y < 0
      || worldPoint.x > this.mapPixelWidth
      || worldPoint.y > this.mapPixelHeight
    ) {
      return;
    }
    ...
    void this.waitForConnection().then(() => {
      if (this.playerData && !this.playerSprite) {
        void this.createPlayerSprite({ ...this.playerData, mode: "avatar" }).then(() => {
          ...
          this.focusSelf(true);
          this.eventBus.emit("user_character_mode_changed", "avatar");
        });
      }
    });
  }
  ...
  private async applyLocalPlayerData(data: { playerId: string; player: UserCharacterData }): Promise<void> {
    ...
    await this.createPlayerSprite(this.playerData);
    this.eventBus.emit("local_user_character_id_changed", {
      previousPlayerId,
  ...
  private async createPlayerSprite(state: UserCharacterData): Promise<void> {
    ...
    if (generation !== this.spriteGeneration || this.playerId !== state.id) {
      return;
    }
  ...
  private async ensureUserCharacterTexture(state: UserCharacterData): Promise<string | null> {
  ...
  private setupKeyboardInput(): void {
    ...
    this.keys = {
      ...
      UP: this.scene.input.keyboard.addKey("UP", false),
      DOWN: this.scene.input.keyboard.addKey("DOWN", false),
      LEFT: this.scene.input.keyboard.addKey("LEFT", false),
      RIGHT: this.scene.input.keyboard.addKey("RIGHT", false),
    };
  }
  ...
  private setupEventListeners(): void {
    this.eventBus.on("set_user_character_mode", this.handleSetUserCharacterMode);
  ...
  private handleKeyboardMovement(delta: number): void {
    ...
    const speed = USER_CHARACTER_SPEED_PX_PER_SEC * (delta / 1000);
    let dx = 0;
    let dy = 0;
  ...
  private cancelClickNavigation(): void {
    if (!this.playerSprite) return;
    this.clickNavigationGeneration++;
  ...
  private async moveTo(x: number, y: number): Promise<void> {
    if (!this.playerSprite) return;
    const navigationGeneration = ++this.clickNavigationGeneration;
    ...
    if (navigationGeneration !== this.clickNavigationGeneration) return;
  ...
  async setMode(mode: UserCharacterMode): Promise<void> {
    ...
    if (mode === "god") {
      ...
      this.spriteGeneration++;
      this.scene.cameras.main.stopFollow();
      this.cameraController.setKeyboardPanEnabled(true);
    } else {
      // 使用 WebSocket 连接时收到的用户角色数据，或从 API 获取
      let state = this.playerData;
      if (!state) {
        try {
          state = this.playerId ? await apiClient.getUserCharacterRuntimeAvatar(this.playerId) : null;
        } catch {
          // 忽略
        }
      }
    ...
    this.eventBus.emit("user_character_mode_changed", mode);
  }
  ...
  destroy(): void {
    ...
    this.eventBus.off("set_user_character_mode", this.handleSetUserCharacterMode);
