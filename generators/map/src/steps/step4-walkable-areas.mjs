...
export async function generateWalkableMap(compressedMapBuffer, userPrompt, worldDesign, save) {
  ...
  const reviewMode = (process.env.STEP4_REVIEW_MODE || "vision").trim().toLowerCase();
  ...
  for (let attempt = 1; attempt <= totalAttempts; attempt++) {
    ...
    if (reviewMode === "programmatic-first") {
      console.log("[Step 4] Programmatic-first mode enabled; skipping Vision review and deferring to grid validation.");
      return {
        buffer: markedBuffer,
        reviewPassed: false,
        attempts: attempt,
    ...
    if (reviewError) {
      console.log(`[Step 4] Review unavailable on attempt ${attempt}, keeping current marked image for downstream programmatic validation.`);
      return {
        buffer: markedBuffer,
        reviewPassed: false,
        attempts: attempt,
