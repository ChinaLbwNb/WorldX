 ...
 * Reads IMAGE_GEN_* env vars. Default: MaaS Gemini 2.5 Flash image model.
 */
...
function getModel() {
  return process.env.IMAGE_GEN_MODEL || DEFAULT_MODEL;
}

function getBaseUrl() {
  return process.env.IMAGE_GEN_BASE_URL || DEFAULT_BASE_URL;
}

function getProvider() {
  return (process.env.IMAGE_GEN_PROVIDER || "").trim().toLowerCase();
}
...
async function postGoogleNativeImage(parts, { apiKey, signal }) {
  return fetch(buildGoogleNativeUrl(apiKey), {
    ...
    headers: {
      ...
      // 同时带上 Bearer 头：兼容云雾等需要 Authorization 的中转（官方 Google 走 ?key= 不受影响）
