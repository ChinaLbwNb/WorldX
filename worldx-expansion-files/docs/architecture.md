...
地图扩展由 React 侧的 `BuildPanel` 发起，是异步 job 模式：

1. BuildPanel 显示东南西北 4 个方向按钮
2. 检查资源 ≥ 50（默认 `mapExpand` 成本），调用 `apiClient.expandMap(direction)`
3. 服务端 `MapExpander.startExpandJob()` 扣除资源，spawn 扩展脚本子进程
4. 客户端 `setInterval(poll, 2000)` 轮询 `getMapExpandJob(jobId)` 获取进度
5. 扩展完成后，服务端 `worldManager.reloadAfterExpansion()` 重载碰撞网格和世界尺寸
6. 客户端刷新 buildState 获取新资源点，刷新页面加载新地图
...
地图扩展完成后，`MapExpander` 调用 `worldManager.reloadAfterExpansion()`，该方法重新执行 `loadCollisionGrid()` 读取更新后的 TMJ 文件，刷新碰撞网格和世界尺寸。这保证了扩展后的新区域立即可通行，无需重启服务。

### 可行走区域拼接

地图扩展时的可行走区域拼接是整个系统最精密的环节，涉及三个层面的对齐：

**像素层 — 接缝融合**：`blendSeam()` 在新旧地图的拼接边界处做 smoothstep 梯度混合。融合宽度为 `max(tileSize, dimension × 0.03)`，在融合区域内逐像素线性插值新旧像素值，权重用 `t² × (3 - 2t)` 平滑。这保证了视觉上不会出现硬边。

**网格层 — 碰撞网格合并**：`mergeGrids(oldGrid, newGrid, dir)` 将两个二维碰撞网格合并。水平扩展时 `mergedW = oldW + newW`，垂直扩展时 `mergedH = oldH + newH`。旧网格和新网格按方向放置到合并网格的对应位置，未覆盖区域默认填充 1（阻塞）。这保证了可行走区域在新旧区域之间自然延续——原图边缘的道路和新区域中与之相连的道路在合并后的网格中保持连通。

**坐标层 — 对象偏移**：`buildCombinedTMJ()` 处理区域和交互对象的坐标偏移。west/north 扩展时旧对象需要平移（旧区域整体向右或向下移动以腾出新区域的空间），east/south 扩展时新对象需要偏移（新区域放在旧区域的右侧或下方）。偏移量使用实际背景像素尺寸（`oldBgWidth`/`newBgWidth`）而非假设尺寸，保证坐标精确。
...
┌──────────────┬──────────────┐
│              │              │
│   旧地图      │   新区域      │  mergedGrid: (W+W')×max(H,H')
│              │              │
└──────────────┴──────────────┘
               ↑
          接缝融合区
...
- **快照系统**：创建/恢复/列出快照
- **扩展后重载**：`reloadAfterExpansion()` 重新读取 TMJ 刷新碰撞网格
...
| MapExpander | generators/map/src/expand-map.mjs | `[Step N]` 6 步进度映射 | `worldManager.reloadAfterExpansion()` |
